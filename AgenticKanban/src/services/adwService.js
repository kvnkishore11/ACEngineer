...
class ADWService {
  ...
  /**
   * Discover pipelines from project structure (simulated)
   */
  async discoverProjectPipelines(projectPath) {
    // This would normally read from agentics/adws/ directory
    // For now, return default pipelines with project-specific variations

    const projectPipelines = [...this.defaultPipelines];

    // Simulate project-specific pipeline discovery
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
          discovered: projectPipelines,
          customConfigPath: `${projectPath}/agentics/adws/`,
          claudeCommandsPath: `${projectPath}/.claude/commands/`,
        });
      }, 500);
    });
  }
  ...
  importPipeline(pipelineConfig) {
    ...
    const { exportedAt, version, ...cleanPipeline } = pipelineConfig;
