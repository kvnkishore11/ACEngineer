...
// take the I wnat to create a kanban style board with 6 stages plan, build, test, review, document, PR, Errored

this should be a loclahost or any  which can select its project with open projects where in the root folder is placed. 

what i am envisioning is that all the commands within .claude/commands are the prmitives like implement, plan, prime, build meaning they are called /shashcommands
...
│   ├── app-docs/.    --> Documentationfor General users of the respository
...
Test - Testing with active monitoring
Review - Code review (CodeRabbit integration)
